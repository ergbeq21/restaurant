...
{#if createNewProdukt}
	<CreateNewProdukt
		bind:open={createNewProdukt}
		categories={data.categories}
		bind:chosenCategoryId
	/>
{/if}
...
<main class="mt-2 mr-5 ml-75 w-full overflow-y-auto p-3">
	...
	<section class="mb-6 flex w-full items-center justify-between">
		...
		<button
			...
			class={`flex items-center gap-2 rounded-lg px-4 py-2 font-medium text-white transition 
                ${data.categories.length == 0 ? 'cursor-not-allowed bg-blue-200' : 'cursor-pointer bg-stone-600 hover:bg-stone-700'}`}
	...
	{#if data.products.length > 0 && data.categories.length > 0}
		{#each data.categories as category}
			<section class="mb-8">
				<h2
					class="mb-4 flex flex-row items-center justify-start gap-4 text-2xl font-bold text-gray-800"
				>
					{category.name}
				</h2>
				<div class="flex gap-4">
					<button
						class="flex h-48 w-48 flex-shrink-0 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-orange-400 bg-orange-50 text-gray-700 transition hover:bg-orange-100"
					...
					>
						...
						<span class="text-center text-base font-semibold"
							>Add new produkt in {category.name}</span
						>
					</button>
